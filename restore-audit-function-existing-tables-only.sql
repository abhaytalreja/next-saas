-- Restore the audit function with correct tables only
-- First, let's check what tables actually exist before creating triggers
    ...
    BEGIN
        ...
        LOOP
            IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = table_name) THEN
                ...
                EXECUTE format('CREATE TRIGGER audit_%I AFTER INSERT OR UPDATE OR DELETE ON %I FOR EACH ROW EXECUTE FUNCTION create_audit_log()', table_name, table_name);
                RAISE NOTICE 'Created audit trigger for %', table_name;
            ELSE
                RAISE NOTICE 'Skipped audit trigger for % (table does not exist)', table_name;
            END IF;
        END LOOP;
    END;
END $$;
...
SELECT 'AUDIT FUNCTION PROPERLY RESTORED FOR EXISTING TABLES ONLY' as status;