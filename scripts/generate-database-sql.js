...
CREATE INDEX idx_projects_${mode === 'none' ? 'user_id' : 'organization_id'} ON projects(${mode === 'none' ? 'user_id' : 'organization_id'});
...
VALUES 
  ('Starter', 'starter', 900, 9000, 
   ARRAY['3 team members', '5 projects', '10GB storage', '10,000 API calls/month', 'Email support'],
   '{"users": 3, "projects": 5, "storage_gb": 10, "api_calls": 10000}'::jsonb,
   true, 1),
  ('Pro', 'pro', 2900, 29000,
   ARRAY['10 team members', '50 projects', '100GB storage', '100,000 API calls/month', 'Priority support', 'Advanced analytics'],
   '{"users": 10, "projects": 50, "storage_gb": 100, "api_calls": 100000}'::jsonb,
   false, 2),
  ('Enterprise', 'enterprise', 0, 0,
   ARRAY['Unlimited team members', 'Unlimited projects', 'Unlimited storage', 'Unlimited API calls', 'Dedicated support', 'Custom domain'],
