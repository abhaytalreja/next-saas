-- Fix users table RLS policies and activity function (safe version)
-- Migration: 003_fix_users_rls_and_activity
-- Date: 2025-01-12

-- 1. Add service role access to users table (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Service role has full access to users" ON users
    FOR ALL
    TO service_role
    USING (true)
    WITH CHECK (true)';
...
-- 2. Allow organization admins to read user profiles when inviting (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'users' 
    AND policyname = 'Organization admins can read user profiles for invitations'
  ) THEN
    EXECUTE 'CREATE POLICY "Organization admins can read user profiles for invitations" ON users
    FOR SELECT
    TO authenticated
    USING (
      EXISTS (
...
-- 3. Update the create_activity function to fix ambiguous column reference
...
-- 4. Add service role access to activities table (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- 5. Handle organization_events table (used in invitation API)
DO $$
BEGIN
  ...
  EXCEPTION
    WHEN undefined_table THEN
      ...
      RAISE NOTICE 'organization_events table does not exist, skipping';
      RETURN;
  END;
  
  -- Add service role access (if not exists)
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
  ...
  -- Allow organization members to create events (if not exists)
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'organization_events' 
  ...
  -- Allow organization members to view events (if not exists)
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'organization_events' 
